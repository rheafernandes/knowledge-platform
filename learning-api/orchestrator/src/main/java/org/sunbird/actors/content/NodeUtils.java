...
public class NodeUtils {
    ...
    public static Map<String, Object> getSerializedMap(Node node, List<String> fields) {
        DefinitionNode definitionNode = DefinitionFactory.getDefinition(node.getGraphId(), node.getObjectType(), "1.0");
        Map<String, Object> metadataMap = node.getMetadata();
        if (CollectionUtils.isNotEmpty(fields))
            filterOutFields(metadataMap, fields);
        Map<String, Object> updatedMetadataMap = metadataMap.entrySet().stream().collect(Collectors.toMap(entry -> handleKeyNames(entry, fields), entry -> convertJsonProperties(entry, definitionNode)));
        Map<String, Object> definitionMap = JavaConversions.mapAsJavaMap(definitionNode.getRelationDefinitionMap());
    ...
    private static Object convertJsonProperties(Map.Entry<String, Object> entry, DefinitionNode definitionNode) {
        List<String> jsonProps = JavaConversions.seqAsJavaList(definitionNode.fetchJsonProps());
